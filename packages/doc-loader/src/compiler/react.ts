...
export function compileReact(code) {
  ...
  traverse(ast, {
    Program: (_path) => {
      ...
      const expressions =
        body.filter((b) => b.type === "ExpressionStatement") || [];
...
export function removeDuplicateImports(allImports): any {
  ...
  const keysImportDefaultSpecifier = Object.keys(
    importsMap.ImportDefaultSpecifier
  );
  const keysImportNamespaceSpecifier = Object.keys(
    importsMap.ImportNamespaceSpecifier
  );
  if (keysImportSpecifier.length) {
    importStringArr = importStringArr.concat(
      keysImportSpecifier.map(
        (k) =>
          `import {${[...importsMap.ImportSpecifier[k]].join(
            ","
          )}} from "${k}";`
      )
    );
  ...
  if (keysImportDefaultSpecifier.length) {
    importStringArr = importStringArr.concat(
      keysImportDefaultSpecifier.map(
        (k) =>
          `import ${[...importsMap.ImportDefaultSpecifier[k]]} from "${k}";`
      )
    );
  }
